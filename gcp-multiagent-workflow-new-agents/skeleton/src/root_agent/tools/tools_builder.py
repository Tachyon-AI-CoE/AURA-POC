...
# Lightweight adapter to expose MCPTool as a callable tool compatible with
# the LlmAgent tooling expectations. The agent framework accepts a callable
# (or other tool types) so providing a __call__ makes MCPTool usable without
# depending on internal BaseTool classes.
class MCPToolAdapter:
    def __init__(self, mcp_tool: MCPTool):
        self._mcp_tool = mcp_tool
        # preserve common metadata used by callers
        self.name = getattr(mcp_tool, "name", "mcp-tool")
        self.description = getattr(mcp_tool, "description", "MCP HTTP tool")

    def __call__(self, method: str = "GET", path: str = "", json: Optional[dict] = None, timeout: int = 10):
        """Callable entrypoint used by the agent when invoking the tool.

        This will forward to MCPTool.request and return the structured dict
        that MCPTool.request() produces.
        """
        return self._mcp_tool.request(method=method, path=path, json=json, timeout=timeout)

    def health(self, timeout: int = 5):
        return self._mcp_tool.health(timeout=timeout)
...
class ToolsBuilder:
    ...
    def create_mcp_tool(
        ...
        """
        try:
            # Build a minimal MCP tool that knows how to call the MCP server
            name = mcp_tool_config.get("name", "gcp-mcp-tool")
            cfg = mcp_tool_config.get("config", {})
            # cfg may include server_url, api_key, base_path, health_path
            mcp_tool = MCPTool(name, cfg)
            # Wrap in adapter so the returned object is callable (satisfies
            # the LlmAgent tooling validator) while still exposing health()
            adapter = MCPToolAdapter(mcp_tool)
            logger.info(
                f"MCP tool '{name}' instantiated for agent {agent_name} (wrapped in adapter)"
            )
            return adapter
        except Exception as e:
            logger.warning(
    ...
    def build_tools_from_yaml_config(
        ...
        for tool_type, tool_configs in tools_config.items():
            # `enabled_tools` is a config flag, not a tool list - ignore it
            if tool_type == "enabled_tools":
                continue
